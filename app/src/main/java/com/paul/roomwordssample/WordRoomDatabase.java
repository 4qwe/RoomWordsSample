...
public abstract class WordRoomDatabase extends RoomDatabase {
    ...
    public static WordRoomDatabase getDatabase(final Context context) { //code für "singleton"
        if (INSTANCE == null) {
            synchronized (WordRoomDatabase.class) {
                if (INSTANCE == null) {
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(), WordRoomDatabase.class,
                            "Word").fallbackToDestructiveMigration().addCallback(sRoomDatabaseCallback).build();
    ...
    private static class PopulateDbAsync extends AsyncTask<Void, Void, Void> {
        ...
        String[] words = {"Elliott", "Smith", "1995"};
